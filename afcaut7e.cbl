...
*-------|----------|-----|----------------------------------------------------*
*G.02.09|2018-02-13| hkn | BMP32 in Nachricht 210 mit Längenangabe
*-------|----------|-----|----------------------------------------------------*
...
*-------|----------|-----|----------------------------------------------------*
*G.02.03|2017-12-12| kl  | Keine Prüfung auf BMP 32 bei WAS2                  *
*       |          |     | wg. ausländischer Karten (wird vom ausl. AS        *
...
*       |          |     | - Daraus resultierende Aufräumarbeiten in C050
*-------|----------|-----|----------------------------------------------------*
...
*-------|----------|-----|----------------------------------------------------*
*G.01.15|2017-05-03| hkn | Überarbeitung AC-Werte
*-------|----------|-----|----------------------------------------------------*
*G.01.14|2017-03-23| hkn | MAC-Prüfung am Ende der ANTWORT-Prüfung
*-------|----------|-----|----------------------------------------------------*
*G.01.13|2017-03-21| hkn | Logging: Nach MAC-Prüfung mit Fehler vom AS
*-------|----------|-----|----------------------------------------------------*
*G.01.12|2017-03-15| hkn | Übersetzung wg. Änderung DUKPT
*-------|----------|-----|----------------------------------------------------*
*G.01.11|2017-02-02| hkn | FCPARM ersetzt durch FCPARAM
*-------|----------|-----|----------------------------------------------------*
*G.01.10|2016-11-08| hkn | Übersetzung wg. Änderung DUKPT
*-------|----------|-----|----------------------------------------------------*
*G.01.09|2016-11-07| hkn | Übersetzung wg. Änderung DUKPT
*-------|----------|-----|----------------------------------------------------*
*G.01.08|2016-11-01| hkn | Fehlende Unterprograme eingefügt
*-------|----------|-----|----------------------------------------------------*
*G.01.07|2016-11-01| hkn | Nur übersetzt, wg. Änderung DUKPT
*-------|----------|-----|----------------------------------------------------*
...
*-------|----------|-----|----------------------------------------------------*
*G.01.04|2016-10-12| hkn | EUROWAG: PAC Umschlüssen und Bilden
*       |          |     | mit DUKPT-Verfahren
*-------|----------|-----|----------------------------------------------------*
...
*       |          |     |
*-------|----------|-----|----------------------------------------------------*
*G.00.06|2016-05-19| hkn | C500-LOGGING: Umsatzscheibung geprüft
*-------|----------|-----|----------------------------------------------------*
...
*-------|----------|-----|----------------------------------------------------*
*G.00.03|2016-04-28| cb  | Neucompile da Änderung im Modul SYSABL1M
*-------|----------|-----|----------------------------------------------------*
...
* definiert werden (z.B. AFCAUT7S-05 für Avia). Die Keys für das
...
*kl20180404 - G.02.10 - Zusätzlicher Searchkey für CARDID = 0
...
*G.01.18 - Anfang - Umgestellt und verschoben
**  ---> BMP 39 - Antwortcode
...
*kl20170519 - G.02.01 - "Katastrophe" beseitigt
...
*    BMP 3 immer an erster Stelle (Handeingabe durch verkürztes BMP 35!)
...
**  +++> und Daten in AS-Struktur bereitstellen
 ...
 C050-99.
     EXIT.
*+++ Das bleibt mal zum Freuen drin +++
******************************************************************
* Holen Daten aus MEMLOG
******************************************************************
*C050-GET-MEMLOG SECTION.
...
*kl20160513 - G.00.04 - Prototyping neues BMP 59 - ertsmal nur für
*                       Antworten vom SHELL-As (AS4SH ...)
*    IF IMSG-MONNAME(4:2) = "SH"
*kl20160517 - G.00.05 - So ein Quatsch ...
*       MOVE IMSG-CF(IMSG-TPTR(59) + 4:6)  TO W-TERMNR
*       MOVE IMSG-CF(IMSG-TPTR(59) + 12:6) TO W-TRACENR
*       MOVE ZEROES                        TO W-NTYPE
*       MOVE ZEROES                        TO W-BMP03
*       MOVE IMSG-CF(IMSG-TPTR(59) + 4:6)  TO W-TRACENR
*kl20160517 - G.00.05 - Ende
*    ELSE
*       alle anderen erstmal wie bisher
*       MOVE IMSG-CF(IMSG-TPTR(59) +  4:8) TO W-TERMNR
*       MOVE IMSG-CF(IMSG-TPTR(59) + 12:6) TO W-TRACENR
*       MOVE 200                           TO W-NTYPE
*       MOVE IMSG-CF(IMSG-TPTR(59) + 18:6) TO W-BMP03
*    END-IF
*    Falls Prototyping ok, geht es so weiter
*    MOVE IMSG-CF(IMSG-TPTR(59)+4:8)    TO W-TERMNR
*    MOVE IMSG-CF(IMSG-TPTR(59) + 12:6) TO W-TRACENR
*    MOVE ZEROES                        TO W-NTYPE
*    MOVE ZEROES                        TO W-BMP03
...
*    IF IMSG-MONNAME(4:2) = "SH"
*       Bei Shell gibt es kein Abwkz und keine Belegnr mehr in BMP59
*       CONTINUE
*    ELSE
*       Alle anderen erst einmal wie gehabt
*       MOVE IMSG-CF(IMSG-TPTR(59) + 18:2) TO W-ABWKZ
*       MOVE IMSG-CF(IMSG-TPTR(59) + 20:4) TO W-BELEGNR
*    END-IF
*kl20160513 - G.00.04 - Ende

*kl20160517 - G.00.05  - Bei SHELL wird nur noch die AS-Tracenr benoetigt
*     MOVE W-MEMLOG-KEY TO MEM-LOGKEY
*    IF IMSG-MONNAME(4:2) = "SH"
*       MOVE W-TRACENR TO MEM-LOGKEY
*    ELSE
*       MOVE W-MEMLOG-KEY     TO MEM-LOGKEY
*    END-IF
*kl20160517 - G.00.05 - Ende
...
*kl20160513 - G.00.04 - Prototyping neues BMP 59 - hier Belegnr aus TS-Anfrage
*                       Antworten vom SHELL-As (AS4SH ...)
*     IF IMSG-MONNAME(4:2) = "SH"
*       Bei Shell gibt es Abwkz und keine Belegnr mehr in BMP59
*       daher aus BMP 3 der TS-Anfrage
*        MOVE W2TS-CF(IMSG-TPTR(03):2)        TO W-ABWKZ
*        MOVE W2TS-CF(IMSG-TPTR(03) + 2:4)    TO W-BELEGNR
*     END-IF
*kl20160513 - G.00.04 - Ende
           ...
           IF IMSG-TBMP(30) = 0
             IF IMSG-TBMP(4) = 1
                MOVE "Ablehnung vom AS ohne BMP 30"
                  TO DATEN-BUFFER1
                MOVE "Transaktion wird mit AC=91 beantwortet"
                  TO DATEN-BUFFER2
*G.01.15 - Anfang
*               MOVE 91 TO W-AC
***
...
*kl20171212 - G.02.03 - Prüfung nicht bei WAS2 wg. ausl. Karten
     IF W-ROUTKZ = 5
        CONTINUE
     ELSE
...
*            Genehmigte EMV-TRansaktion, aber KEINE Chipdaten vom AS
             ...
             MOVE "Keine Chipdaten vom AS bei genehmigter EMV-TX"
                   TO DATEN-BUFFER1
*                Ausgabe Transaktionsschlüssel nur, wenn vorhanden
             IF  W2TS-TBMP(41) = 1
             AND W2TS-TPTR(41) > 0
             AND W2TS-TLEN(41) > 0
             AND W2TS-TBMP(11) = 1
...
*    IF  W-AC-AS = 185
*        IF  IMSG-TBMP(62) = 0
*            MOVE "AC=185 ohne BMP 62"                    TO DATEN-BUFFER1
...
*G.01.14 - Anfang - Prüfung wurde verschoben
     IF W-AC > ZEROS
        EXIT SECTION
     ELSE

**  ---> MAC prüfen, sofern vorhanden
      SET MAC-NO TO TRUE
...
******************************************************************
* spezielle Behandlung für das ENI-AS früher AGYP
******************************************************************
 D315-ENI SECTION.
 D315-00.
...
*AC 87 und BMP 63 mit 0 Produkten ans TS zurück schicken
...
*                       das spätestens im Artikel-Mapper ab ...
